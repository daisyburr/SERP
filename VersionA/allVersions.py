#opened 06/28/17 
#script to bypass using multiple versions, orders, and seperate scripts
...
print subj_id
neutral_per_block = 3
negative_per_block = 7
#Make random order for a subject
if run == 1:
    image_numbers = pd.read_csv("/Users/whalenlab/Desktop/Experiments/SERP/VersionA/allImages.csv"); 
    subject_order = pd.DataFrame()
    random_neutral_images = image_numbers.query("Condition == 'Neutral'").sample(frac=1)
    assert len(random_neutral_images) % neutral_per_block == 0, "Number of neutral images not divisible by 3!"
       
    random_negative_images = image_numbers.query("Condition == 'Negative'").sample(frac=1)
    assert len(random_negative_images) % negative_per_block == 0, "Number of negative images not divisible by 3!"
    
    for neut_idx, neg_idx in zip(xrange(0,28,3),xrange(0,64,7)):
        current_run_neutral = random_neutral_images.iloc[neut_idx,neut_idx+neutral_per_block,:]
        current_run_negative = random_negative_images.iloc[neut_idx,neut_idx+negative_per_block,:]
        
        
    
    num_neutral = len(random_neutral_images)
    subject_order = pd.DataFrame(
    {
    'Subject': [subj_id]*len(image_numbers),
    'Image': 
    ...
    run_data = pd.read_csv(os.path.join("/Users/whalenlab/Desktop/Experiments/SERP",str(subj_id)+'_run_'+str(run)+'.csv'))
